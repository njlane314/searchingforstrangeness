...
bool is_relative(const std::string& path) {
  ...
  if (path.size() > 1 && path[1] == ':') return false; // Windows drive
...
std::string resolve_under(const std::string& base, const std::string& value) {
  ...
  if (!is_relative(value)) return value;
...
const image::PlaneImage* find_view(std::vector<image::PlaneImage> const& planes,
  ...
  for (auto const& plane : planes) {
    if (plane.view == static_cast<int>(view)) return &plane;
  }
  return fallback;
}
...
InferenceProductionModule::InferenceProductionModule(fhicl::ParameterSet const& p)
  ...
  , defaultWrapper_{resolve_under(assetsBaseDir_,
                     p.get<std::string>("DefaultWrapper", "scripts/inference_wrapper.sh"))}
...
void InferenceProductionModule::produce(art::Event& e) {
  ...
  if (scratch.empty()) {
    if (const char* env = std::getenv("_CONDOR_SCRATCH_DIR")) scratch = env;
    else scratch = ".";
  }

  char buf[4096];
  std::string absoluteScratch = realpath(scratch.c_str(), buf) ? std::string(buf) : scratch;

  auto perfProduct = std::make_unique<image::InferencePerfProduct>();
  auto featureProduct = std::make_unique<std::vector<image::RandomFeatures>>();

  for (auto const& cfg : models_) {
    std::string assets = cfg.assets.empty() ? assetsBaseDir_ : cfg.assets;
    std::string wrapper = cfg.wrapper.empty() ? defaultWrapper_ : cfg.wrapper;
    ...
    auto result = image::InferenceProduction::runInferenceDetailed(
      detector_images, absoluteScratch, assets, cfg.arch, weights, wrapper, assets,
      cfg.want_cls, cfg.want_seg);
