...
GraphEngine::GraphEngine(Params params)
    : fParams(std::move(params)) {}

void GraphEngine::set_params(const Params& params) {
    fParams = params;
}

const Params& GraphEngine::params() const {
    return fParams;
}

GraphEngine::SpatialGrid::SpatialGrid(const std::vector<HitNode>* hits,
                                                 float cell_size)
    : fHits(hits)
...
void GraphEngine::SpatialGrid::reset(const std::vector<HitNode>* hits,
                                                float cell_size) {
    fHits = hits;
    fCellSize = std::max(cell_size, 0.05f);
    build();
...
std::vector<int> GraphEngine::SpatialGrid::queryRadius(const Vec2& p,
                                                                  float radius) const {
    std::vector<int> out;
...
std::vector<int> GraphEngine::SpatialGrid::queryAABB(float xmin,
                                                                float xmax,
                                                                float ymin,
                                                                float ymax) const {
    std::vector<int> out;
...
bool GraphEngine::SpatialGrid::hasOccupancy(const Vec2& p,
                                                       float radius,
                                                       int exclude_hit_a,
                                                       int exclude_hit_b,
                                                       int exclude_comp_a,
...
GraphEngine::SpatialGrid::CellKey
...
void GraphEngine::SpatialGrid::build() {
    fBins.clear();
    if (!fHits) return;
...
float GraphEngine::merged_interval_length(std::vector<std::pair<float, float>> intervals,
                                                     float lo,
                                                     float hi) {
...
bool GraphEngine::clip_segment_to_box(const Vec2& a,
                                                 const Vec2& b,
                                                 float xmin,
                                                 float xmax,
                                                 float ymin,
...
bool GraphEngine::solve2x2(float a00,
                                      float a01,
                                      float a10,
                                      float a11,
                                      float b0,
...
DerivedScales GraphEngine::make_scales(const PlaneGeometry& geometry) const {
    DerivedScales out;
...
GraphEngine::LocalGeom GraphEngine::compute_local_geom_for_hit(
    int i,
...
void GraphEngine::compute_local_geometry(std::vector<HitNode>& hits,
                                                    const SpatialGrid& grid,
                                                    const DerivedScales& scales) const {
...
float GraphEngine::sample_support_fraction(const Vec2& a,
                                                      const Vec2& b,
                                                      const SpatialGrid& grid,
                                                      const std::vector<HitNode>& hits,
                                                      float radius,
...
std::vector<GraphEngine::CandidateEdge>
...
float GraphEngine::edge_cost(const CandidateEdge& edge,
                                        const std::vector<HitNode>& hits,
                                        const DerivedScales& scales) const {
...
std::vector<GraphEngine::ComponentSummary>
...
Vec2 GraphEngine::component_axis(const ComponentSummary& comp,
...
Vec2 GraphEngine::local_axis_at_hit(int hit_idx,
...
float GraphEngine::start_asymmetry(int center_hit_idx,
                                              const Vec2& axis,
                                              const ComponentSummary& comp,
                                              const std::vector<HitNode>& hits,
                                              const DerivedScales& scales) const {
...
void GraphEngine::choose_component_start(ComponentSummary& comp,
...
std::vector<GraphEngine::CompGraphEdge>
...
std::vector<float> GraphEngine::dijkstra_component_graph(
...
bool GraphEngine::inside_plane_bounds(const Vec2& p,
                                                 const DerivedScales& scales) const {
    return (p.x >= scales.wire_min && p.x <= scales.wire_max &&
            p.y >= scales.drift_min && p.y <= scales.drift_max);
}

void GraphEngine::corridor_metrics(const Vec2& a,
                                              const Vec2& b,
                                              const SpatialGrid& grid,
                                              const std::vector<HitNode>& hits,
...
GraphEngine::ParentCandidate GraphEngine::best_parent_candidate(
...
void GraphEngine::classify_components(
    std::vector<ComponentSummary>& comps,
    const std::vector<HitNode>& hits,
    const SpatialGrid& grid,
    const DerivedScales& scales,
...
bool GraphEngine::fit_common_secondary_vertex(const std::vector<ComponentSummary*>& group,
                                                         Vec2& vertex,
...
void GraphEngine::merge_detached_activities(std::vector<ComponentSummary>& comps) const {
    for (auto& comp : comps) {
...
Result GraphEngine::run(const PlaneInput& input) const {
    Result out;
    out.input = input;
