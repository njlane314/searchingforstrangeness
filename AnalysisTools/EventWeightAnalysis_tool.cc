#ifndef ANALYSIS_EVENTWEIGHT_CXX
#define ANALYSIS_EVENTWEIGHT_CXX
    ...
    void EventWeightAnalysis::analyseEvent(art::Event const& evt, bool is_data) {
        ...
        for(auto& thisTag : vecTag){
            ...
            if(eventweights_handle.isValid()) {
                std::cout << " [ EventWeightAnalysis ]" << " isValid! " << std::endl;
                ...
                //std::cout << " [ EventWeightAnalysis ]" << " continue... " << std::endl;
                ...
                for(std::map<std::string, std::vector<double>>::iterator it=evtwgt_map.begin(); it!=evtwgt_map.end(); ++it){
                    ...
                    if (keyname.find("All") != std::string::npos) {
                        std::cout << " [ EventWeightAnalysis ]" << " Entering Genie ALL variation number " << GenieCounter << " " <<keyname << std::endl;
                    ...
                    // if this is a ppfx multisim variation
                        ...
                        std::cout << " [ EventWeightAnalysis ]" << " Entering PPFX variation number " << PPFXCounter << " " <<keyname << std::endl;
                        for(unsigned int i = 0; i < it->second.size(); ++i) {
                        ...
                        std::cout << " [ EventWeightAnalysis ]" << " Entering Reweighted PPFX variation number " << PPFXCounter << " " <<keyname << std::endl;
                        for(unsigned int i = 0; i < it->second.size(); ++i) {
                            if ( (i + (100 * PPFXCounter) ) < _vecWeightsPPFXD.size())
                                _vecWeightsPPFXD[i + (100 * PPFXCounter) ] *= it->second[i];
                        }
        ...
        if (!_SaveAllFlux) {
        //std::cout << " [ EventWeightAnalysis ] " << "flux-all weight vector now has " << _vecWeightFlux.size() << " elements" << std::endl;
        _mapWeight.insert( std::pair<std::string,std::vector<double> >("flux_all",_vecWeightFluxD) );
        }
        // store geinie-all map
        //std::cout << " [ EventWeightAnalysis ] " << "flux-all weight vector now has " << _vecWeightsGenie.size() << " elements" << std::endl;
        _mapWeight.insert( std::pair<std::string,std::vector<double> >("All_UBGenie",_vecWeightsGenieD) );

        // store reinteraction map
        //std::cout << " [ EventWeightAnalysis ] " << "reinteraction (G4) weight vector now has " << _vecWeightsReint.size() << " elements" << std::endl;
        _mapWeight.insert( std::pair<std::string,std::vector<double> >("reint_all",_vecWeightsReintD) );

        // store PPFX map
        //std::cout << " [ EventWeightAnalysis ] " << "PPFX weight vector now has " << _vecWeightsPPFX.size() << " elements" << std::endl;
