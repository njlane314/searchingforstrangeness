...
private:
    ...
    static int countActivePixels(const std::vector<int32_t> &coords);
};
...
int ImageAnalysis::countActivePixels(const std::vector<int32_t> &coords) {
    return static_cast<int>(coords.size() / 2);
}
