...
TVector3 ProjectToWireView(const float input_x, const float input_y, const float input_z, const PandoraView pandora_view) {
    ...
    return TVector3(x_coord, 0.f, pandora_view == TPC_VIEW_U ? YZtoU(y_coord, z_coord) : pandora_view == TPC_VIEW_V ? YZtoV(y_coord, z_coord)
                                                                                                                    : YZtoW(y_coord, z_coord));
}
...
TVector3 GetPandoraHitPosition(const art::Event &e, const art::Ptr<recob::Hit> hit, const PandoraView pandora_view) {
    ...
    return TVector3(x_coord, 0.f, pandora_view == TPC_VIEW_U ? YZtoU(xyz.Y(), xyz.Z()) : pandora_view == TPC_VIEW_V ? YZtoV(xyz.Y(), xyz.Z())
                                                                                                                    : YZtoW(xyz.Y(), xyz.Z()));
}
} // namespace common
