...
class VisualiseEventAnalyser : public art::EDAnalyzer
{
public:
    explicit VisualiseEventAnalyser(fhicl::ParameterSet const &pset);
...
VisualiseEventAnalyser::VisualiseEventAnalyser(fhicl::ParameterSet const &pset)
    : EDAnalyzer{pset}
    ...
    , _BacktrackTag{pset.get<art::InputTag>("BacktrackTag", "gaushitTruthMatch")}
{
    ...
    for (const auto &entry : entry_coll) 
    {
        if (entry.size() == 3) {
            ...
            throw cet::exception("VisualiseEventAnalyser") << "-- Invalid TargetEvents format. Each entry must contain exactly three integers (Run, SubRun, Event).";
        }
    }
}

void VisualiseEventAnalyser::beginJob() 
{}

void VisualiseEventAnalyser::analyze(art::Event const &e)
{
    ...
    mf::LogInfo("VisualiseEventAnalyser") << "-- Visualising event: Run " << e.run() 
                                           << ", SubRun " << e.subRun() 
                                           << ", Event " << e.event();
...
void VisualiseEventAnalyser::visualiseEvent(const art::Event &e, const std::string &filename)
{
    ...
    if (!e.getByLabel(_MCTproducer, mc_particle_handle))
        throw cet::exception("VisualiseEventAnalyser") << "failed to find any mc particles in event" << std::endl;
    ...
    if (!e.getByLabel(_HitProducer, hit_handle))
        throw cet::exception("VisualiseEventAnalyser") << "failed to find any hits in event" << std::endl;
...
void VisualiseEventAnalyser::get_limits(const std::vector<float>& wire_coord_vec, const std::vector<float>& drift_coord_vec,
...
void VisualiseEventAnalyser::endJob() 
{}

DEFINE_ART_MODULE(VisualiseEventAnalyser)
