...
bool PatternCompleteness::filter(const art::Event &e, const Signature& sig, const Type& type, common::PandoraView view)
{
    if(_verbose)
        std::cout << "Checking PatternCompleteness in view " << view << " for signature " << type << std::endl;
    ...
    if (tot_sig_hit < _sig_hit_thresh){
        if(_verbose)
            std::cout << "Signature " << type << " failed PatternCompleteness with too few hits" << std::endl;
        return false;
    }

    if(_verbose)
        std::cout << "Signature " << type << " passed PatternCompleteness in plane " << view << std::endl;
