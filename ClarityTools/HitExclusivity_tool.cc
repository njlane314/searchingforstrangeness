...
bool HitExclusivity::filter(const art::Event &e, const Signature& sig, const Type& type, common::PandoraView view)
{
    if(_verbose)
        std::cout << "Checking HitExclusivity in view " << view << " for signature " << std::endl;
    ...
    for (const auto& mcp_s : sig) {
        ...
        if (sig_q_exclusive / sig_q_inclusive < _sig_exclus_thresh){
            if(_verbose)
                std::cout << "Signature " << " failed HitExclusivity in plane " << view << std::endl;
            return false;
        }
    ...
    if(_verbose)
        std::cout << "Signature " << " passed HitExclusivity in plane " << view << std::endl;
