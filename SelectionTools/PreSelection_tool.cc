...
// containment and then this
// probably only use hits and extent here and place a loose cut
namespace selection
{
    class PreselectionBDTTool : public SelectionToolBase {
    public:
        explicit PreselectionBDTTool(const fhicl::ParameterSet& pset);
        ~PreselectionBDTTool();
    ...
    PreselectionBDTTool::PreselectionBDTTool(const fhicl::ParameterSet& pset) {
        configure(pset);
    }

    PreselectionBDTTool::~PreselectionBDTTool() {
        if (bdt_) XGBoosterFree(bdt_);
    }

    void PreselectionBDTTool::configure(fhicl::ParameterSet const& pset) {
        ...
        if (!trainingMode_) {
            ...
            if (xgtest != 0) throw cet::exception("PreselectionBDTTool") << "XGBoost error: " << xgtest;
        }
    }

    bool PreselectionBDTTool::selectEvent(art::Event const& e, const std::vector<common::ProxyPfpElem_t>& pfp_pxy_v) {
    ...
    std::vector<float> PreselectionBDTTool::computeFeatures(const art::Event& e) {
        art::Handle<std::vector<recob::Hit>> hitHandle;
        e.getByLabel(hitLabel_, hitHandle);
        if (!hitHandle.isValid()) {
            mf::LogWarning("PreselectionBDTTool") << "No hits found with label: " << hitLabel_;
    ...
    float PreselectionBDTTool::predictBDT(const std::vector<float>& features, BoosterHandle bdt, int nTrees) {
        int xgtest = -1;
        DMatrixHandle dmat;
        ...
        if (xgtest != 0) throw cet::exception("PreselectionBDTTool") << "XGBoost predict error: " << xgtest;
        float result = *out_result;
        XGDMatrixFree(dmat);
        return result;
    }

    void PreselectionBDTTool::writeTrainingData(const std::vector<float>& features, int label) {
    ...
    void PreselectionBDTTool::setBranches(TTree* _tree) {
    ...
    void PreselectionBDTTool::resetTTree(TTree* _tree) {
        nHitsU_ = nHitsV_ = nHitsW_ = 0;
        totalChargeU_ = totalChargeV_ = totalChargeW_ = 0.0f;
        wireRangeU_ = wireRangeV_ = wireRangeW_ = 0.0f;
        timeRangeU_ = timeRangeV_ = timeRangeW_ = 0.0f;
    ...
    DEFINE_ART_CLASS_TOOL(PreselectionBDTTool)
}